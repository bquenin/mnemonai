...
install_from_release() {
	log_info "Installing mnemonai from GitHub releases..."
	...
	local version="${MNEMONAI_VERSION:-}"

	if [ -z "$version" ]; then
		log_info "Fetching latest release..."
		local latest_url="https://api.github.com/repos/bquenin/mnemonai/releases/latest"
		local release_json
	...
	# Download URL
	local archive_name="mnemonai-${platform}.tar.gz"
	local download_url="https://github.com/bquenin/mnemonai/releases/download/${version}/${archive_name}"
	...
	local checksum_file="mnemonai-${platform}.sha256"
	local checksum_url="https://github.com/bquenin/mnemonai/releases/download/${version}/${checksum_file}"
	...
	local install_dir="${MNEMONAI_INSTALL_DIR:-}"
	...
	if [ -f "$install_dir/mnemonai" ]; then
		local existing_version
		existing_version=$("$install_dir/mnemonai" --version 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
	...
	local tmp_binary="$install_dir/mnemonai.tmp.$$"

	if [[ -w "$install_dir" ]]; then
		cp mnemonai "$tmp_binary"
		chmod +x "$tmp_binary"
		mv -f "$tmp_binary" "$install_dir/mnemonai"
	else
		if ! sudo cp mnemonai "$tmp_binary"; then
			log_error "Failed to install to $install_dir (sudo required)"
			exit 1
		fi
		sudo chmod +x "$tmp_binary"
		sudo mv -f "$tmp_binary" "$install_dir/mnemonai"
	fi

	# Remove macOS quarantine attribute if present
	if [[ "$(uname -s)" == "Darwin" ]] && command -v xattr &>/dev/null; then
		xattr -d com.apple.quarantine "$install_dir/mnemonai" 2>/dev/null || true
	fi

	log_success "mnemonai installed to $install_dir/mnemonai"
...
verify_installation() {
	...
	if [ ! -x "$install_dir/mnemonai" ]; then
		log_error "mnemonai binary not found or not executable at $install_dir/mnemonai"
		exit 1
	fi

	# Test the binary works
	if ! "$install_dir/mnemonai" --version &>/dev/null; then
		log_error "mnemonai binary exists but failed to run"
		exit 1
	fi

	log_success "mnemonai is installed and ready!"
	echo ""
	"$install_dir/mnemonai" --version
	echo ""

	echo "Get started:"
	echo "  mnemonai        # browse conversation history"
	echo "  mnemonai --help # see all options"
	echo ""
	echo "Documentation: https://github.com/bquenin/mnemonai"
	echo ""
}
